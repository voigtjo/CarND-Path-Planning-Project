			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					if ( car_ahead ) {
						// Car is ahead
						if ( !car_left && lane > 0 ) {
							// if there is no car left and there is a left lane: Change lane left
							lane--;
						} else if ( !car_righ && lane != 2 ){
							// if there is no car right and there is a right lane: Change lane right
							lane++;
						} else {
							speed_diff -= MAX_ACC;
						}
					} else {
						if ( lane != 1 ) { // if we are not on the center lane: Back to center
